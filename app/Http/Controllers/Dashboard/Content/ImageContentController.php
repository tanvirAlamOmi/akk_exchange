...
class ImageContentController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('dashboard.contents.image-contents.multiple-image-content-inputs',compact('editRow','section'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (count($request->uploadedImages) > 0) {
            ...
            foreach ($request->uploadedImages as $k => $uploadedImage) {
                ...
                $image_name = $this->imageHelper->uploadImageResizing($encodeType, $uploadedImage, 'content-images', null, 5248576, $width, $height, $imageContent->title, ['width' => null, 'height' => null, 'thumbStorageName' => null] );
                // get max size alert
    ...
    {
        $editRow = ImageContent::with(['section'])->find($id);
        $section = $editRow->section;
        return view('dashboard.contents.image-contents.image-content-inputs',compact('editRow','section'));
    }
